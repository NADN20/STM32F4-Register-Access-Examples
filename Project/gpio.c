...
void GPIO_ClockEnable (unsigned int * gpio_x)
{
  ...
  {
   *RCC_GPIO_clock |= (1<<0x05);
  }
  else if (gpio_x == GPIO_G)
  {
   *RCC_GPIO_clock |= (1<<0x06);
  }
  else if (gpio_x == GPIO_H)
  {
   *RCC_GPIO_clock |= (1<<0x07);
  }
   else
  {
     if(gpio_x == GPIO_I)       
...
void GPIO_DeInit(unsigned int * gpio_x)
{
  ...
  {
   *RCC_GPIO_reset |=(1<<0x05);    
   *RCC_GPIO_reset &=~(1<<0x05); 
  }
  else if (gpio_x == GPIO_G)
  {
   *RCC_GPIO_reset |=(1<<0x06);    
   *RCC_GPIO_reset &=~(1<<0x06); 
  }
  else if (gpio_x == GPIO_H)
  {
   *RCC_GPIO_reset |=(1<<0x07);    
   *RCC_GPIO_reset &=~(1<<0x07); 
  }
  else
  {
